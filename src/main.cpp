...
const double BASE_FREQ = 261.626;
...
int main(int argc, char *argv[])
{
    ...
    AudioDevice audio = AudioDevice(audio_callback, buffers, SAMPLE_RATE, NULL, 0, 0, ASDLT, 2, SAMPLES);
    ...
    while (true)
    {
        ...
        if (buffers->ready == false)
        {
            ...
            for (int i = 0; i < length; i++, i++)
            {
                ...
                for (Note &n : notes)
                {
                    double dTone = (adsr.env(n.val, n.state) * sin(2.0 * M_PI * (BASE_FREQ * pow(TWELFTHROOTOFTWO, n.key - 1)) * (dTime - n.timeOn))); //Value between -1 and 1 scaled to min and max
