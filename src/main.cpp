...
void audio_callback(void *user_data, Uint8 *raw_buffer, int bytes)
{
    ...
    for (int i = 0; i < length; i++, sample_nr++)
    {
        ...
        buffer[i] += (Sint16)(AMPLITUDE * sin(2.0f * M_PI * (261.626f * pow(TWELFTHROOTOFTWO, n.key - 1)) * time)); // render 441 HZ sine wave
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        {
            if (event.key.keysym.sym == SDLK_ESCAPE && event.type == SDL_KEYDOWN) return 0; // Exit condition
            ...
            {
                auto find = std::find_if(notes.begin(),notes.end(), [key](Note& n){ return n.key == key; } );
                if(find == notes.end()){
                    Note n;
                    n.key = key;
                    n.active = true;
